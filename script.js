// script.js - FIXED: Refresh elements only on refresh/reopen, not on first answer
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (savedAnswer) {
    ...
    // Show refresh elements ONLY if this is a refresh/reopen (refreshCount > 0)
    // AND the first answer has already been done
    if (refreshCount > 0 && firstAnswerDone) {
      console.log("Showing refresh elements - refresh count:", refreshCount);
      ...
      console.log("Not showing refresh elements - first visit after answer");
      // Ensure they're hidden on first answer view
  ...
  function saveAnswerAndShowLetter(answer) {
    ...
    // Mark that first answer is done
    localStorage.setItem(FIRST_ANSWER_KEY, 'true');
    
    // Set refresh count to 0 initially (first answer, not a refresh)
    refreshCount = 0;
